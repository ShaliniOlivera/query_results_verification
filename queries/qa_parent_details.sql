...
MAX(CASE WHEN co1.description = 'Mother' THEN COALESCE(us1.home_phone, 'None') END) AS parent_one_home_phone,
    ...
    COALESCE(
        MAX(CASE WHEN co1.fk_school = 2 AND co1.description = 'Mother' THEN co_occup1.description END),
        MAX(CASE WHEN co1.description = 'Mother' THEN p1.occupational_title END)
    ) AS parent_one_occupation,
    MAX(CASE WHEN co1.description = 'Mother' THEN p1.working_status END) AS parent_one_working_status,
    COALESCE(MAX(CASE WHEN co1.description = 'Mother' THEN code_mother_workplace_staff1.description END), '') AS parent_one_workplace_association,
    ...
    MAX(CASE WHEN co2.description = 'Father' THEN COALESCE(us2.home_phone, 'None') END) AS parent_two_home_phone,
    ...
    COALESCE(
        MAX(CASE WHEN co2.fk_school = 2 AND co2.description = 'Father' THEN co_occup2.description END),
        MAX(CASE WHEN co2.description = 'Father' THEN p2.occupational_title END)
    ) AS parent_two_occupation, 
...
LEFT JOIN code co1 
    ON cr1.fk_relation = co1.id AND co1.active = 1 AND co1.fk_school = 2
LEFT JOIN code co_occup1 
    ON co_occup1.label = p1.occupational_title AND co_occup1.active = 1 AND co_occup1.fk_school = 2
LEFT JOIN code code_mother_workplace_staff1 
    ...
    AND code_mother_workplace_staff1.active = 1
...
LEFT JOIN code co2 
    ON cr2.fk_relation = co2.id AND co2.active = 1 AND co2.fk_school = 2
LEFT JOIN code co_occup2 
    ON co_occup2.label = p2.occupational_title AND co_occup2.active = 1 AND co_occup2.fk_school = 2

-- Address Logic (Prioritizing First Applicant's Address)
LEFT JOIN address ad1 
    ON ad1.fk_parent = p1.id 
    AND EXISTS (
        SELECT 1 FROM child_relation cr 
        WHERE cr.fk_child = ch.id 
          AND cr.fk_parent = p1.id 
          AND cr.type = 'Applicant'
    )
    AND ad1.active = 1
...
LEFT JOIN address ad2 
    ...
    AND NOT EXISTS ( 
        ...
        WHERE fk_parent = p1.id
    )
    AND ad2.active = 1
LEFT JOIN `user` us1 ON p1.fk_user = us1.id
LEFT JOIN `user` us2 ON p2.fk_user = us2.id
