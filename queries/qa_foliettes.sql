SELECT DISTINCT 
    ...
    -- Ensure the GROUP_CONCAT output is wrapped with JSON_ARRAY()
IFNULL((
    SELECT JSON_ARRAYAGG(
        JSON_OBJECT(
            "type", cai.type, 
            "source_path", cai.url, 
            "display_order", cai.index,   -- Use cai.index as the display order
            "created_at", 
    ...
    ORDER BY cai.index ASC  -- Ensure the media entries are ordered by cai.index
