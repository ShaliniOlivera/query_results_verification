-- Sha's class_class data
SELECT ch.id,
ce.code AS centre_code,
cla.id AS class_id,
le.code AS level_code,
ifnull(ccla.`from`,'') AS `from`,
ccla.`to`,
ifnull(ccla.`created_at`,'') AS created_at,
ifnull(ccla.`updated_at`,'') AS updated_at
FROM class cla
...
INNER JOIN `child` ch ON ch.id = ccla.fk_child
