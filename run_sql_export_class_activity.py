...
# File paths
sql_file = os.path.join(sql_dir, "qa_class_activity.sql")
proc_files = {
    ...
    "create_temp_classes": os.path.join(sql_dir, "create_temp_classes.sql"),
}

# Generate timestamped filename
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
output_file = os.path.join(result_dir, f"qa_class_activity_result_{timestamp}.csv")
...
try:
    ...
    with conn.cursor() as cursor:
        ...
        procedures = ["create_temp_child", "create_temp_child_class", "create_temp_classes"]
        for proc in procedures:
            try:
                cursor.execute(f"CALL {proc}();")
                conn.commit()
            except pymysql.err.OperationalError as e:
                print(f"⚠️ Skipping {proc}() due to error: {e}")
        ...
        if os.path.exists(sql_file):
            ...
            try:
                ...
                if row_count > 0:
                    ...
                    # ✅ Ensure NULL values are preserved as None instead of empty strings
                    df_sql = df_sql.where(pd.notna(df_sql), None)

                    # ✅ Ensure JSON fields (e.g., `children`) correctly handle NULLs
                    if "children" in df_sql.columns:
                        df_sql["children"] = df_sql["children"].apply(lambda x: None if x in [None, "null", ""] else x)
        ...
        # Step 7: Save query result to CSV with explicit NULL values
        df_sql.to_csv(output_file, index=False, na_rep="NULL")
